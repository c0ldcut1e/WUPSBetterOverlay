...
public:
    ...
    explicit CThread(int32_t iAttr, int32_t iPriority = 16,
                     int32_t iStackSize            = 0x8000,
                     CThread::Callback callback    = nullptr,
        ...
        if (pThread && pThreadStack) {
            OSCreateThread(pThread, &CThread::threadCallback, 1, (char *) this,
                           pThreadStack + iStackSize, iStackSize, iPriority,
                           iAttributes);
    ...
    ~CThread() { shutdownThread(); }

    static CThread *create(CThread::Callback callback, void *callbackArg,
                           int32_t iAttr      = eAttributeNone,
                           int32_t iPriority  = 16,
                           int32_t iStackSize = 0x8000) {
        return (new CThread(iAttr, iPriority, iStackSize, callback,
                            callbackArg));
    }

    [[nodiscard]] virtual OSThread *getThread() const { return pThread; }

    virtual void executeThread() {
        if (pCallback) { pCallback(this, pCallbackArg); }
    }
    ...
    bool resumeThread() {
        if (!isThreadSuspended()) { return false; }
    ...
    void shutdownThread() {
        if (pThread && !(iAttributes & eAttributeDetach)) {
            if (isThreadSuspended()) { resumeThread(); }

            OSJoinThread(pThread, nullptr);
        }

        if (pThreadStack) { free(pThreadStack); }

...
DECL_FUNCTION(void, GX2GetCurrentScanBuffer, GX2ScanTarget scanTgt,
              GX2ColorBuffer *colBuffer) {
    real_GX2GetCurrentScanBuffer(scanTgt, colBuffer);
    if (scanTgt == GX2_SCAN_TARGET_TV)
        memcpy(&lastTVColBuffer, colBuffer, sizeof(GX2ColorBuffer));
    else
...
void drawIntoColorBuffer(const GX2ColorBuffer *colBuffer,
                         GX2ScanTarget scanTgt) {
    real_GX2SetContextState(gCtxState);
    GX2SetColorBuffer((GX2ColorBuffer *) colBuffer, GX2_RENDER_TARGET_0);
...
DECL_FUNCTION(void, GX2SetTVBuffer, void *buffer, uint32_t bufferSize,
              int32_t renderMode, GX2SurfaceFormat surfaceFmt,
              GX2BufferingMode bufferingMode) {
    tvSurfaceFmt = surfaceFmt;
    return real_GX2SetTVBuffer(buffer, bufferSize, renderMode, surfaceFmt,
                               bufferingMode);
}

DECL_FUNCTION(void, GX2SetDRCBuffer, void *buffer, uint32_t bufferSize,
              int32_t renderMode, GX2SurfaceFormat surfaceFmt,
              GX2BufferingMode bufferingMode) {
    drcSurfaceFmt = surfaceFmt;
    return real_GX2SetDRCBuffer(buffer, bufferSize, renderMode, surfaceFmt,
                                bufferingMode);
}

DECL_FUNCTION(void, GX2CopyColorBufferToScanBuffer,
              const GX2ColorBuffer *colBuffer, GX2ScanTarget scanTgt) {
    gDrawAllowed = true;

    GX2ColorBuffer _colBuffer = *colBuffer;
    drawIntoColorBuffer(&_colBuffer, scanTgt);
...
DECL_FUNCTION(void, GX2Init, uint32_t attrs) {
    ...
    if (!init) {
        ...
        GX2Invalidate(GX2_INVALIDATE_MODE_CPU, gCtxState,
...
DECL_FUNCTION(void, GX2MarkScanBufferCopied, GX2ScanTarget scanTgt) {
    ...
    GX2ColorBuffer *colBuffer = (scanTgt == GX2_SCAN_TARGET_TV)
                                        ? &lastTVColBuffer
                                        : &lastDRCColBuffer;
    drawIntoColorBuffer(colBuffer, scanTgt);
...
DECL_FUNCTION(int32_t, VPADRead, VPADChan chan, VPADStatus *input,
              uint32_t count, VPADReadError *err) {
    VPADReadError realErr;
    int32_t res = real_VPADRead(chan, input, count, &realErr);
...
WUPS_MUST_REPLACE(GX2SetContextState, WUPS_LOADER_LIBRARY_GX2,
                  GX2SetContextState);
WUPS_MUST_REPLACE(GX2SetupContextStateEx, WUPS_LOADER_LIBRARY_GX2,
...
WUPS_MUST_REPLACE(GX2MarkScanBufferCopied, WUPS_LOADER_LIBRARY_GX2,
                  GX2MarkScanBufferCopied);
WUPS_MUST_REPLACE(GX2SwapScanBuffers, WUPS_LOADER_LIBRARY_GX2,
                  GX2SwapScanBuffers);
